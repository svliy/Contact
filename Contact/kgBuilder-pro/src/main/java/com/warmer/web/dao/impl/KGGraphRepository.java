...
import com.warmer.web.request.ShortPathQuery;
...
public class KGGraphRepository implements KGGraphDao {
    ...
    public HashMap<String, Object> getShortPath(ShortPathQuery query) {
        ...
        List<KgDomain> domaindata = knowledgeGraphService.getDomainByName(query.getDomain());
        nr.put("domaindata", domaindata);
        String nodeSql = "MATCH (startNode: `"+query.getDomain()+"` {name:'"+ query.getStartNode() +"'}), (endNode:`"+query.getDomain()+"` {name:'"+ query.getEndNode() +"'}) call apoc.algo.dijkstra(startNode, endNode, null, 'distance') YIELD path return path";
